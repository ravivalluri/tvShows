...
export class TVShowsService {
  ...
  getDirector(slug: string) {
    ...
    return this.http
      .get(`${environment.apiUrl}/director/${slug}`)
  ...
  getSeries(slug: string, worst: boolean) {
    ...
    return this.http.get(`${environment.apiUrl}/episodes/${params}`).pipe(
      map((series: any) => {
  ...
  vote(
    ...
    episodeId: string,
    rating: number
  ): Observable<{ rating: number; ratingCount: number }> {
    return this.http.post<{ rating: number; ratingCount: number }>(
      `${environment.apiUrl}/vote`,
      {
        seriesId,
        episodeId,
        rating
