...
export class SeriesPageComponent implements OnInit {
  ...
  getSeries(slug: string, worst: boolean, seasons: boolean) {
    const request = seasons
      ? this.TVShowsService.getSeasons(slug)
      : this.TVShowsService.getSeries(slug, worst);
    request.subscribe(
      series => {
        this.series = series;
  ...
  episodeImage(episode) {
    ...
    if (episode.filename) {
      return `https://thetvdb.com/banners/${episode.filename}`;
    }
  ...
  setMeta(series, worst: boolean, seasons: boolean) {
    const description = `A list of the ${worst ? 'lowest' : 'highest'} rated ${
      seasons ? 'seasons' : 'episodes'
    } of ${
      series.seriesName
    }, ranked by thousands of ratings from fans of the series`;
    const title = `The ${worst ? 'Worst' : 'Best'} ${
      seasons ? 'Seasons' : 'Episodes'
    } of ${series.seriesName}`;
    const image = `https://cdn.episode.ninja/file/episodeninja/${
      series.id
    }.jpg`;

    this.title.setTitle(
      `${worst ? 'Worst' : 'Best'} ${series.seriesName} ${
        seasons ? 'Seasons' : 'Episodes'
      } | episode.ninja`
    );
    this.meta.addTags([
    ...
    this.authService.isAuthenticated().subscribe((loggedIn: boolean) => {
      if (loggedIn) {
        ...
        const disposable = this.simpleModalService
          .addModal(SignupPromptComponent)
          .subscribe(() => {
